...
void *receive_info(void *arg)
{
    while (1) {
        ...
        if (recvfrom(sock, packet, sizeof(packet), 0, (struct sockaddr *)&otheraddr, &addr_size) < 0) {
            perror("recvfrom");
            continue;
        }

        int sender_id = ntohl(packet[0]);
        int neighbor_id = ntohl(packet[1]);
        int new_cost = ntohl(packet[2]);

        if (sender_id < 0 || sender_id >= N || neighbor_id < 0 || neighbor_id >= N) {
            continue;
        }

        pthread_mutex_lock(&lock);
        costs[sender_id][neighbor_id] = new_cost;
        costs[neighbor_id][sender_id] = new_cost;
        pthread_mutex_unlock(&lock);
    }
}
